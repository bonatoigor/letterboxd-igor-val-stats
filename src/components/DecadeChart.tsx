...
export default function DecadeChart({ data }: DecadeChartProps) {
  ...
  return (
    <div className="bg-lb-surface rounded-lg p-5 md:p-6">
      ...
      <div className="flex items-end gap-0.5 sm:gap-1 md:gap-2" style={{ height: '10rem' }}>
        {data.map((d) => {
          const barHeight = max > 0 ? (d.count / max) * 100 : 0;
          return (
            <div key={d.name} className="flex-1 flex flex-col items-center justify-end gap-0.5 sm:gap-1 h-full min-w-0">
              <span className="text-[8px] sm:text-[10px] text-lb-text tabular-nums">{d.count}</span>
              <div
              ...
              />
                <span className="text-[9px] sm:text-xs md:text-sm font-medium text-lb-text truncate w-full text-center mt-0.5 sm:mt-1">
                  {d.name}
                </span>
            </div>
          );
